...
def process_pilot(filename, airstart, start, end, name):
    ...
    df['distance_bin'] = (df['distance'] // 10) * 10 # Round every 10m
...
def plot_pilot(df, name):    
    ...
    #sns.lineplot(data=df, x=df.index, y=value, marker='o', label=f'{name} {value}')
...
process_pilot('dump/roger.csv', airstart, d_start, d_end, 'Roger')
